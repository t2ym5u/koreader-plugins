...
| `cave_solvability_check.lua` | cave | **Fixed (2026-07-28), partial at n=7/8-medium** — literal full-grid comparison; zero uniqueness verification pre-fix (severe: n=6/hard 0/10, n=8/easy 1/15). Needed two sound pruning rules to be tractable at all: bidirectional future-connectivity (mirrors numberlink's per-color reachability check, applied to both the shaded and unshaded regions) plus per-clue visibility bounds (a clue's still-open ray has a computable min/max reach; prune immediately if the target falls outside it) — together cut worst-case latency from ~56s to sub-second in most cases. Fixed generate+verify style: escalate the clue-keep ratio in bounded steps per cave shape before drawing a fresh one. n=6: 100% unique at every difficulty. n=7/n=8: mostly 100%, with n=7/medium (~65%) and n=8/medium (~75%) as honest partial spots (falls back to the best structurally-valid candidate when verification can't conclude in time — never worse than pre-fix). Worst case ~7.9s at n=8/hard. |
| `tatami_solvability_check.lua` | tatami | **Audited, no bug found (2026-07-28)** — unlike every other game in this audit, provably always unique rather than merely measured as such: the already-constructive generator (fixed 2026-07-27) produces one of exactly 2 possible domino tilings per size, and those 2 tilings were confirmed to share **zero cells in common** — so any non-empty clue reveal instantly and unambiguously identifies the true solution. No fix needed; this file is a regression guard for that "zero shared cells" invariant, not a fix verification. |
...
**Tier 3, Phase 5 (cave, tatami) completed 2026-07-28 — this closes out
the whole Tier 3/4 roadmap** (only galaxies, deliberately excluded pending
its separate robustness bug, and the 33 out-of-scope games' lightweight
spot-check remain). cave needed the standard fix — zero uniqueness
verification, same shape as nurikabe/hitori/masyu — just with an unusually
hard-to-verify dual constraint (both the shaded AND unshaded regions must
stay connected simultaneously). tatami is the one genuine exception in
this whole audit: it needed no fix at all. Its generator was already made
fully constructive in an earlier session (exactly 2 possible domino
tilings per size, period), and checking those 2 tilings directly showed
they share zero cells in common — meaning uniqueness could be *proven*
analytically in a few lines, rather than *measured* by building a full CSP
solver and running hundreds of trials. Worth remembering for any future
"is this puzzle type at risk" triage: if a generator is already fully
constructive with a small, enumerable solution space, checking the
solution space directly can be far cheaper and more conclusive than
reaching for the standard uniqueness-counter playbook.
