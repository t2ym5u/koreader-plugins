-- Uniqueness regression check for numbrix.koplugin.
--
...
--   luajit spec/solvability_audits/numbrix_solvability_check.lua
--
-- Background: before 2026-07-22, numbrix revealed a flat `given_ratio` of
-- random intermediate cells with zero uniqueness verification (see the
-- original `numbrix_uniqueness_diagnostic.lua` finding: fine on Easy, but
-- 33-53% ambiguous on Medium/Hard). Fixed by digging cells one at a time
-- (like sudoku-common's hole-digging) -- start fully revealed, tentatively
-- hide each intermediate cell in random order, verify with
-- `countCompletions` (an independent Hamiltonian-path-completion counter,
-- separate code from board.lua's own copy) after each removal, put the cell
-- back if that broke uniqueness. Re-run this after any future change to
-- board.lua's `generate`/`countCompletions`/`GIVEN_RATIOS` to catch a
-- regression.
--
-- Exit code is non-zero if any difficulty drops below its threshold.

package.preload["gettext"] = function() return function(s) return s end end
package.path = "game-common/?.lua;numbrix.koplugin/?.lua;" .. package.path
local Board = require("board")

local DIRS = { {-1,0},{1,0},{0,-1},{0,1} }

-- Counts Hamiltonian-path completions (up to `limit`) consistent with the
-- given fixed cells. Deliberately separate from board.lua's own
-- countCompletions -- cross-checks the production gate rather than testing
-- a fix against itself.
