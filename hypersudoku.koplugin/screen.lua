...
local HyperSudokuBoardWidget = lrequire("board_widget")
...
function HyperSudokuScreen:buildLayout()
    ...
    self.board_widget = HyperSudokuBoardWidget:new{
        board              = self.board,
...
function HyperSudokuScreen:updateStatus(message)
    local status
    if message then
        status = message
    else
