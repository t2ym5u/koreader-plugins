...
local HYPER_REGIONS = board_module.HYPER_REGIONS
...
local HyperSudokuBoardWidget = BaseBoardWidget:extend{
    board = nil,
}

function HyperSudokuBoardWidget:paintTo(bb, x, y)
    ...
    local region_color = Blitbuffer.COLOR_GRAY_D
    for _, region in ipairs(HYPER_REGIONS) do
        ...
        bb:paintRect(rx, ry, rw, rh, region_color)
    end
