...
-- Draw a line between two points via pixel steps
...
-- ---------------------------------------------------------------------------
-- BetweenLinesBoardWidget
-- ---------------------------------------------------------------------------

local BetweenLinesBoardWidget = BaseBoardWidget:extend{
    board = nil,
}

function BetweenLinesBoardWidget:paintTo(bb, x, y)
    ...
    -- Draw between-lines BEFORE grid lines
    local line_color  = Blitbuffer.COLOR_GRAY_B
    local line_thick  = math.max(2, math.floor(cell * 0.07))
    local bulb_radius = math.max(3, math.floor(cell * 0.18))

    for _, line in ipairs(self.board.lines or {}) do
        local cells = line.cells
        if #cells >= 2 then
            -- Draw line segments connecting consecutive cell centers
            for i = 1, #cells - 1 do
                local r1 = cells[i].r
                local c1 = cells[i].c
                ...
                drawThinLine(bb, cx1, cy1, cx2, cy2, line_color, line_thick)
            end
            -- Draw filled circle (bulb) at both endpoints
            local ep1 = cells[1]
            local ep2 = cells[#cells]
            local bx1 = math.floor(x + (ep1.c - 0.5) * cell)
            local by1 = math.floor(y + (ep1.r - 0.5) * cell)
            local bx2 = math.floor(x + (ep2.c - 0.5) * cell)
            local by2 = math.floor(y + (ep2.r - 0.5) * cell)
            drawCircle(bb, bx1, by1, bulb_radius, line_color)
            drawCircle(bb, bx2, by2, bulb_radius, line_color)
        end
    end
...
return BetweenLinesBoardWidget
